var mainLayer = cc.LayerColor.extend({
    ...
    init:function ()
    {
        ...
        //添加分数
        var sp_label=cc.Sprite.create(s_img20);
        sp_label.setAnchorPoint(cc.p(0,0.5));
        sp_label.setScale(0.7);
        sp_label.setPosition(cc.p(0,this.winsize.height*0.97));
        this.addChild(sp_label,2);
        this.sp_gameScore=cc.LabelAtlas.create(this.gameScore,s_img21,25,35,'0');
        this.sp_gameScore.setAnchorPoint(cc.p(0,0.5));
        this.sp_gameScore.setColor(cc.c3(245,245,50));
        this.sp_gameScore.setPosition(cc.p(this.winsize.width*0.25,this.winsize.height*0.966));
    ...
    addGameOverUIs:function()
    {
        ...
        menu.setPosition(cc.p(this.winsize.width*0.5,this.winsize.height*0.38));
        ...
        guanzhu.setPosition(cc.p(this.winsize.width*0.5,this.winsize.height*0.25));
        guanzhu.setTag(204);
        ...
        _over.setPosition(cc.p(this.winsize.width*0.5,this.winsize.height*0.80));
        _over.setTag(210);
        ...
        var particle=cc.ParticleSystem.create(s_img22);
        particle.setPosition(cc.p(this.sp_plane.getPositionX(),this.sp_plane.getPositionY()+this.sp_plane.getContentSize().height*0.15));
        //particle.setAutoRemoveOnFinish(true);
        particle.setRotation(125);
        particle.setTag(205);
        this.addChild(particle,2);

        var _score1=cc.Sprite.create(s_img12);
        _score1.setPosition(cc.p(this.winsize.width*0.5,this.winsize.height*0.55));
        _score1.setTag(206);
        _score1.setColor(cc.c3(0,0,0));
        this.addChild(_score1,2);

        var _score2=cc.Sprite.create(s_img12);
        _score2.setPosition(cc.p(this.winsize.width*0.32,this.winsize.height*0.55));
        _score2.setTag(207);
        _score2.setColor(cc.c3(0,0,0));
        this.addChild(_score2,2);

        var _score3=cc.Sprite.create(s_img12);
        _score3.setPosition(cc.p(this.winsize.width*0.68,this.winsize.height*0.55));
        _score3.setTag(208);
        _score3.setColor(cc.c3(0,0,0));
        this.addChild(_score3,2);

        var rank="";
        if(this.gameScore<600)
        {
            rank="喂，开飞机不要睡觉啊";
        }
        else if(this.gameScore>=600&&this.gameScore<1200)
        {
            rank="驾照考试作弊者";
        }
       else if(this.gameScore>=1200&&this.gameScore<2400)
        {
            _score2.setColor(cc.WHITE);
            rank="初级";
        }
       else if(this.gameScore>=2400&&this.gameScore<3600)
        {
            _score2.setColor(cc.WHITE);
            rank="中级";
        }
       else if(this.gameScore>=3600&&this.gameScore<5400)
        {
            _score1.setColor(cc.WHITE);
            _score2.setColor(cc.WHITE);
            rank="高级";
        }
        else if(this.gameScore>=5400&&this.gameScore<7200)
        {
            _score1.setColor(cc.WHITE);
            _score2.setColor(cc.WHITE);
            rank="特级";
        }
        else if(this.gameScore>=7200&&this.gameScore<10800)
        {
            _score1.setColor(cc.WHITE);
            _score2.setColor(cc.WHITE);
        ...
        else if(this.gameScore>=10800&&this.gameScore<14400)
        {
            _score1.setColor(cc.WHITE);
            _score2.setColor(cc.WHITE);
            _score3.setColor(cc.WHITE);
        ...
        //得分描述
        var percent=(this.gameScore/15000*100).toFixed(2);
        if(percent>=100)percent=99.99;
        var str="你安全行驶了"+this.gameScore+"米.\n击败了朋友圈"+percent+"%的对手.\n点击右上角[分享到朋友圈]给好友玩玩.";
        var finalScore=cc.LabelTTF.create(str, "黑体",23,cc.size(500,150), cc.TEXT_ALIGNMENT_CENTER);
        finalScore.setColor(cc.c3(0, 0, 0));
        finalScore.setAnchorPoint(cc.p(0.5,0.5));
        finalScore.setPosition(cc.p(this.winsize.width*0.5,this.winsize.height*0.635));
        finalScore.setTag(209);
        this.addChild(finalScore,2);
        var str="我驾驶飞机安全行驶了"+this.gameScore+"米,击败了朋友圈"+percent+"%的对手,快来挑战我吧!";
        //分享内容
        document.title = window.wxData.desc = str;
        document.title = window.wxFriend.desc = str;
    },
        ...
        if (this.isGameOver==false)
        {
            ...
            for(var i=300;i<=400;i++)
            {
                ...
                if(_enemy)
                {
                    ...
                    //var ex=_enemy.getPositionX();
                    //var ey=_enemy.getPositionY();
                    //var eh=_enemy.getContentSize().width*0.4;
                    //var px=this.sp_plane.getPositionX();
                    //var py=this.sp_plane.getPositionY();
                    //var ph=this.sp_plane.getContentSize().width*0.4;
                    if(cc.rectIntersectsRect(_enemyBox,_planeBox))//Math.sqrt(Math.pow(Math.abs(ex-px),2)+Math.pow(Math.abs(ey-py),2))<=eh+ph
                    {
                        this.isGameOver=true;
            ...
            for(var i=401;i<=410;i++)
            {
                ...
                if(_mis)
                {
                    ...
                    var _planeBox=this.sp_plane.getBoundingBox();
                    if(cc.rectIntersectsRect(_misBox,_planeBox))//cc.rectIntersectsRect(_misBox,_planeBox)
                    {
                        this.isGameOver=true;
                        this.sp_plane.stopAllActions();
                        this.addGameOverUIs();
            ...
            //刷新分数
            this.gameScore+=1;
            this.sp_gameScore.setString(this.gameScore);
        }
    },
